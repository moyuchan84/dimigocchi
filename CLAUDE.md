...
- [x] **P7 콘텐츠 채우기 — 입시 가이드 실정보 보강 1차 (2026-07-31)** *(P7은 로드맵상 "지속" 단계라 이후에도 계속 갱신됨 — 아래는 1차 작업 기록)*
  - **2027학년도 신입생 입학전형요항이 확정 발표됨(2026.07.30)** — 학교 공식 사이트에서 확정 PDF를 직접 확보해 `src/content/schedule/2027.json`을 `confirmed: true` + 실제 날짜(진로적성특별전형/일반전형 각각의 원서접수·발표일)로 전면 갱신. `guide/index.astro`의 확정/예정 배너가 하드코딩 문구였던 것을 `schedule` 컬렉션을 읽어 `guide/schedule.astro`와 동일한 로직으로 분기하도록 수정(코드 변경, 배너가 콘텐츠 상태와 어긋나지 않게 하기 위함). `src/lib/checklist.ts`의 요강 확정 여부에 의존하던 `tentative` 플래그 5개(now-4/summer-4/before-apply-1/before-apply-5/before-interview-4) 해제
  - `src/content/guide/`에 4개 섹션 신규 작성(`contact`/`campus-life`/`application-process`/`personal-statement`) + 기존 4개 보강(`departments`에 해킹방어과 교육과정/동아리/관련 대회 상세 — 요구사양서 6.5 이행, `admission-types`/`evaluation`/`special-admission`에 확정 배점표 반영). 10개 → 14개 섹션으로 `order` 전면 재배치(코드 변경 없음, `listGuideSections()`가 전부 동적 조회)
  - 원서접수 절차/필요서류/지원자격/중복지원 여부/자기소개서 작성 가이드/기숙사·통학·학교시설/학교 공식 문의채널(카카오톡 1:1 입학상담) 등 기존에 없던 정보 카테고리를 신규 섹션으로 채움. 확정 요강 PDF·학과소개·생활관소개·학교시설소개 등 학교 공식 페이지를 1차 출처로 우선 사용했고, 공식으로 확인되지 않는 항목(정확한 학비 금액 등)은 나무위키를 보조 출처로 표시하거나 문의처 링크로 대체(핵심 원칙 4 — 추측 금지)
  - `npm run build`로 스키마 통과·경고 0건 확인 예정(아래 참고)
- **다음 작업**: P7은 계속 진행되는 단계다 — 이 1차 작업 이후에도 이론/문제/면접 콘텐츠 보강이나 학과별 상세 심화(정확한 동아리 목록·진학/취업 통계 등, 이번엔 공식 확인이 안 돼 보류함) 여지가 남아 있다. 그 다음은 P8 최종 QA/배포 — 아래 "개발 로드맵" 참고
...
- **입시 가이드 14섹션** (`/guide` 내 앵커; P2에서 이식한 원본 9섹션 + P7에서 추가한 4섹션 + 출처). 순서(=frontmatter `order`) 그대로: `school-character` → `contact`(P7 신규) → `departments` → `campus-life`(P7 신규) → `admission-types` → `application-process`(P7 신규) → `evaluation` → `personal-statement`(P7 신규) → `aptitude-test` → `interview` → `special-admission` → `timeline` → `checklist` → `sources`(번호 없는 부록)
...
4. **미확정 정보 표시**: 확정되지 않은 일정/배점 관련 콘텐츠에는 반드시 "예정(가안)" 배지와 확인 시점을 표기하고, 날짜를 추측해 넣지 않는다. (2027학년도 신입생 입학전형요항은 2026-07-30 확정 발표되어 `schedule/2027.json`의 `confirmed`가 `true`로 전환됐다 — 이 원칙은 다음 학년도 요강처럼 향후 다시 미확정 상태가 될 콘텐츠에 계속 적용된다.)
